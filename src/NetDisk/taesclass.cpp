...
//封装接口，单次最多加密长度1024的文本
QByteArray TAesClass::AesEncrypt(QByteArray plainText)
{
    if(plainText.size() > 1024)
        return QByteArray();
    int ret = 0;
    char outbuff[2048];
    ret = OnAesEncrypt((LPVOID)plainText.data(), (DWORD)plainText.size(), (LPVOID)outbuff);
    return QByteArray(outbuff, ret);
}

QByteArray TAesClass::AesUncrypt(QByteArray cipherText)
{
    if(cipherText.size() > 1024)
        return QByteArray();
    int ret = 0;
    char outbuff[2048];
    ret = OnAesUncrypt((LPVOID)cipherText.data(), (DWORD)cipherText.size(), (LPVOID)outbuff);
    return QByteArray(outbuff, ret);
}
